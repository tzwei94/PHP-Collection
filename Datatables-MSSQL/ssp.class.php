...
class SSP
{
     ...
     */
    public static function limit($request, $table, $primaryKey)
    {
        $limit = '';
        ...
        // if (isset($request['start']) && $request['length'] != -1) {
        //     $limit = "ORDER BY " . $table . "." . $primaryKey . " OFFSET " . intval($request['start']) . " ROWS FETCH NEXT " . intval($request['length']) . " ROWS ONLY;";
        // }
        ...
        if (isset($request['columns'])) {
            for ($i = 0, $ien = count($request['columns']); $i < $ien; $i++) {
                ...
                if ($requestColumn['searchable'] == 'true' && $str != '') {
                    // Range Search
                    $pos = strpos($str, 'Range:');
                    $is_range_special = false;
                    if ($pos !== false && $pos === 0) {
                        $is_range_special = true;

                        $range = explode('<>', substr($str, 6));
                        $range[0] = trim($range[0]);
                        $range[1] = trim($range[1]);

                        if ($range[0] != '' && $range[1] == '') {
                            // Return all range newer than start range
                            $binding = SSP::bind($bindings, $range[0], PDO::PARAM_STR);
                            $columnSearch[] = "" . $column['db'] . " >= " . $binding;
                        } else if ($range[0] == '' && $range[1] != '') {
                            // Return all range prior to the end range
                            $binding = SSP::bind($bindings, $range[1], PDO::PARAM_STR);
                            $columnSearch[] = "" . $column['db'] . " <= " . $binding;
                        } else if ($range[0] != '' && $range[1] != '') {
                            // Return all range within range
                            $binding = SSP::bind($bindings, $range[0], PDO::PARAM_STR);
                            $binding2 = SSP::bind($bindings, $range[1], PDO::PARAM_STR);
    ...
    public static function simple($request, $conn, $table, $primaryKey, $columns)
    {
        ...
        $limit = self::limit($request, $table, $primaryKey);
